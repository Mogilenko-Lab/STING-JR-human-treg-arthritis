...
Paired within donor across n = 6, `WT_heat_up` is higher in synovial fluid in every sorted state (Treg dz = +2.18, 95% CI 0.71–3.65, p = 0.003; Tcon +3.16; CD8 +3.26) — **but so is the down arm, harder in Treg and CD8** (+3.50; +4.27), leaving the directional composite flat in Treg (dz = −0.38, p = 0.39) and negative in CD8 (−4.63). Arms anti-correlated by construction cannot both mark the same cells if those cells carry the temperature response, so this channel reports a shared non-directional component — general activation or expression breadth lifting large rank-based scores — and is **not** corroboration of direction. The donor-pseudobulk enrichment keeps the arms apart as expected; where they disagree, it has standing.
...
Read this one, not the per-cell shift, next to donor-pseudobulk results: it shares their unit of replication. It remains **annotation tier** — six donors, uncorrected across readouts, on scores that were carried in rather than re-derived — and it never substitutes for the pseudobulk enrichment. For the mouse anchor specifically it does not corroborate it either, for the reason given above; treat the two mouse arms here as a diagnostic of the scoring channel rather than as a second line of evidence for the temperature program. The hypoxia, effector-Treg and heat-shock readouts carry no such internal contradiction and are read as written. Nothing here contributes a row to the cross-dataset effect-size accumulator. Claim tier L3.
