...
`mthi_*` artifacts are it: they characterise that pocket of cells, establish that it is a
...
The pocket retains Treg identity, holds real cells at lower depth (median 1,338 genes against a
200-gene floor), and carries no doublet flag. It is 98% synovial fluid and 69% one donor, and its
blood arm is essentially empty, so it supports no paired contrast of its own. It therefore stays
inside the main Treg pseudobulk, pooled with the rest of the Treg gate.
...
mitochondrial-high effector-low cluster (leiden 16). Right panel: %mt against eTreg for every
Treg, as a hexbin on log density, with pocket cells overlaid in orange. Both panels label the two
clusters with the numbering `mthi_cluster_enrichment.csv` uses. High mitochondrial content and
effector identity are separable here, so %mt alone defines no pocket.
...
Median gene count in the pocket is 1,338, far above the floor. Reduced depth is the expected
corollary of a high mitochondrial fraction. `score_apoptosis` (rbc −0.14) and `score_HSP` (−0.22)
both sit below normal Treg. No pocket cell carries a `predicted_doublet` flag. `doublet_score`
went unpopulated in this run, a Scrublet gap, so the doublet evidence rests on the flag.
...
exclusion filters are unioned in.
...
holding no saved cell carries OR 0 and p 1 by construction, so read those rows for their %mt and
depth.
...
6.0e-7). Both mitochondrial-high groups fall below Tcon (−0.014) and CD8 (−0.037) on that
