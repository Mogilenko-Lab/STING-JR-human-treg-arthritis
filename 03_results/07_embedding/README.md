...
on every panel and enters the rule nowhere. Asking the anchor score to both select cells and
measure enrichment in them is the circularity this design exists to avoid, and the hooks are
orthogonal biology for exactly that reason.
...
signature concentrates, alone. The anchor is drawn as annotation; the source table gives
per-lineage medians.
*Source* `tables/hook_factor_substrate.parquet` · `02_analysis/scripts/07_embedding_viz.py`.
...
admitted them, with non-drafted cells left grey. A design preview rather than a committed cohort.
*Source* `tables/hook_factor_substrate.parquet` · `02_analysis/scripts/07_embedding_viz.py`.
...
the rule and the cut point it resolved to — effector P90 = 0.1385; within-Treg %mt P97.5 = 10.03
plus an eTreg-median and 400-gene viability floor. `n_cells` and `frac_all_cells` size it.
...
**The union is carried by the sort gate and reduces to it in no way.** Of 33,113 union cells,
...
One row per frozen lineage; the source of the signature-overlay panel. Each signature contributes
a `median_` and a `mean_` column, so a mean far above the median flags a skewed minority rather
than a shifted lineage — Treg `score_eTreg` is the clear case (median −0.0031, mean +0.0240), a
small effector-high tail, which is what the effector hook is built to catch.
...
FOXP3 is detected in 79.7% of sorted Treg against 3.8% of Tcon and 1.0% of CD8; CD8A in 92.7% of
CD8 against 0.2% of Treg; IL7R in 87.7% of Tcon against 42.7% of Treg. IL2RA (66.8%), CTLA4
