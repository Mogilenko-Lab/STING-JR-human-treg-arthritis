...
cap("source_hash_manifest.csv",
  sprintf(paste("The CollecTRI human regulon table read across the compartment boundary is",
                "pinned at sha256 %s..., so a change to that network halts this stage. Without",
                "the pin, both nulls would move in silence."), substr(NET_SHA, 1, 8)),
  "verify_source_hash",
  paste0("unbiased_enrichment.tf_network.path = ", NET_PATH), NET_PATH,
  paste("One row per cross-compartment source: `source_label` is the name this stage refers to it",
        "by, `source_path` is repository-root-relative, `sha256` the digest of the bytes actually",
        "read. The first run writes the pin; every later run verifies against it and stops on a",
        "mismatch. This stage keeps its own pin, so a moved network stops both stages."))
...
cap("ulm_engine_validation.csv",
  ...
  paste("One row. `score_closed_form` against `score_decoupler` is a like-for-like comparison on",
        ...
        "zero for a non-target, so the fit has a closed form costing one sparse matrix-vector",
        "product for all factors at once, which is what makes tens of thousands of null fits",
        "feasible. `gate` reads `pass` only when `spearman` reaches `gate_min_spearman`; the run",
        "stops otherwise. This is a guard, and it satisfies the compartment's standing requirement",
        "that an engine swap be shown by rank correlation to be a method change."))

cap("rewiring_null.csv",
  sprintf(paste("Held against random regulons that share its size and every one of its targets'",
                "in-degrees, HIF1A's CollecTRI-ULM score of %.2f clears the null at empirical",
                "p %.3f, z %.2f, and the null's centre sits at %.2f, so most of the score is what",
                "any regulon of that size and promiscuity profile earns on this contrast. The null",
                "does not single HIF1A out: REL clears it harder (p %.3f, z %.2f) on %d targets,",
                "NFKB1 is indistinguishable (p %.3f), and EPAS1 sits exactly on its own null",
                "(obs %.2f against a null mean of %.2f, p %.2f)."),
  ...
  paste("One row per focus factor. `obs_score` and `obs_rank` are the observed signed-network",
        ...
        "every target's in-degree stay invariant and only the assignment of targets to factors is",
        "randomised — which is why `null_mean` sits far above zero and why beating this null means",
        "more than beating a random gene set of matched size. `mean_target_indeg` is the observed",
        "regulon's average target promiscuity, the property this null holds fixed. `p_empirical` is",
        "(draws at or above obs + 1)/(draws + 1), so it never returns exactly zero, and `z_vs_null`",
        "standardises the gap. Because size is preserved exactly, `null_rank_median` and",
        "`null_rank_best` share the `n_tfs_scored` denominator with `obs_rank`. Annotation tier:",
        "this is a statement about the regulon's target set, and protein activity is untested."))
...
cap("rewiring_null_draws.csv",
  ...
  paste("One row per (draw, factor). `score` is that draw's closed-form ULM value and `rank` its",
        ...
        "one Markov chain advanced between samples by trades, so consecutive rows are decorrelated",
        "and dependent by construction. This is the distribution `rewiring_null.csv` summarises;",
        "read it when a summary statistic needs checking against the shape it came from."))

cap("signflip_null.csv",
  sprintf(paste("Permuting the experimental design with the annotation held fixed, the observed",
                ...
                "being the one that does not (p %.3f). What it establishes is that the contrast",
                "itself survives relabelling."),
  ...
  paste("One row per focus factor. The two tissue labels are swapped within a donor and the whole",
        "limma-voom contrast is refitted, which is the exchangeability a paired design licenses and",
        "which leaves the correlation between genes intact, where permuting gene labels would",
        "destroy it. `n_paired_donors` donors carry both arms, so `n_configurations` = 2^that, and",
        "every one is enumerated: the test is exact and no seed enters it. `n_ge_obs` counts",
        "configurations scoring at or above the observed, the observed one included, and",
        "`p_exact_one_sided` is that count over `n_configurations`. `p_floor` is",
        "1/`n_configurations`, and `at_resolution_floor` is TRUE when only the observed",
        "configuration reaches the observed score, i.e. the p-value is as small as this many donors",
        "can make it. Flipping every donor negates the contrast, so the configurations come in",
        "sign-symmetric pairs and `null_mean` sits near zero. The gene set is filtered once on the",
        "observed design and held fixed, so scores are comparable across configurations; voom is",
        "recomputed per configuration, because its weights depend on the design."))
...
cap("signflip_null_draws.csv",
  ...
  paste("One row per (configuration, factor). `donors_flipped` names the donors whose two tissue",
        "labels were swapped, pipe-delimited, and `is_observed` marks the configuration that flips",
        "none — the published contrast. `score` and `rank` are that configuration's closed-form ULM",
        "value and its position among every factor scored. Configurations are enumerated in a fixed",
        "order, so this table is byte-stable across runs. Read it for the null's shape, and for the",
        "observed row being the extreme one for most factors."))

cap("null_ladder.csv",
  sprintf(paste("Set side by side, the %d nulls order by what they hold fixed, and the conclusion",
                "weakens along that order: HIF1A clears a size-and-expression-matched random",
                "regulon by a wide margin, clears a size-and-promiscuity-matched real-degree",
                "rewiring by a narrow one, and is one of %d focus factors that max out the exact",
                "design permutation. Each rung is published beside the others because no single",
                "rung supports a claim alone."),
          length(unique(ladder$null)), sum(sf$at_resolution_floor)),
  "bind_rows",
  ...
  sprintf(paste("One row per (factor, null), ordered weakest-to-strongest by what the null holds",
                "constant. `holds_fixed` names that and `permutes` names what is randomised, which",
                "together bound what a p-value from that row can mean. `obs` is the same observed",
                "CollecTRI-ULM score in every row for a given factor; only the reference",
                "distribution changes, so the columns are comparable down a factor's block and the",
                "movement in `p_empirical` is the whole content of the table. `null_q95` is empty",
                "for the sign-flip rung, whose %d configurations do not support a stable upper",
                "percentile. The two random-regulon rungs are re-read from their own published",
                "table."),
          n_cfg))
