...
def main() -> None:
    ...
    save_overview(
        ...
        finding=(
            ...
            f"NES {float(inter['nes'].min()):.4f} to {float(inter['nes'].max()):.4f} on 6 "
            f"testable genes and clears pooled FDR {fdr:g} in "
            f"{'none' if n_inter_sig == 0 else n_inter_sig} of the three cell states, so at that "
            "size it carries no direction here."
        ),
        ...
        how_to_read=(
            ...
            "count with the score: an arm of 6 testable genes and one of 130 are not equally "
            "resolved. The adjusted p is Benjamini-Hochberg pooled across every set that "
            "population's sweep tested; the same-stem source table also carries the "
            "per-collection value over the three arms alone, and the two agree except for "
            "Interaction_up in CD8, 0.035 per-collection against 0.172 pooled. A score of this "
