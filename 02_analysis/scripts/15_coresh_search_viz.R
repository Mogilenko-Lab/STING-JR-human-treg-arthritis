...
save_overview(
  ...
  finding = sprintf(
    paste("Across the %s public human datasets searched, the JIA synovial-fluid-versus-blood up-arm",
          ...
          "throughout on a query roughly three times larger, so pctVar is comparable within a",
          "gate and the two rankings are read as two separate searches."),
  ...
  how_to_read = paste(
    ...
    "ranges, because pctVar is comparable within a query size and the relaxed gate's query",
    "is far larger. Exploratory tier: this is a ranking of public datasets."),
...
save_overview(
  ...
  finding = sprintf(
    paste("In the Treg synovial-fluid-versus-blood ranking the %d modules split both ways",
          ...
          "much of it is the query that seeded it:",
          "Spearman rho = %.2f between NES and seed fraction, on a median seed fraction of only",
          "%.0f%%. The enrichment therefore reports seed content back to itself as much as it",
          "reports public biology, which is why this tier supports no claim."),
  ...
  how_to_read = paste(
    ...
    "dataset itself. Columns are the three sorted populations, each scored",
