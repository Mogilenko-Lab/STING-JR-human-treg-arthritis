...
save_overview(
  ...
  finding = sprintf(paste0("Tested one donor at a time, %d of the fourteen PROGENy footprints ",
                           ...
                           "footprints that separate carry both directions."),
  ...
  how_to_read = paste(
    ...
    "reading. The activity score is computed on expression centred within a population, so",
    "a difference is comparable between pathways of the same population; across",
    "populations it carries no meaning, and a Treg point sitting further right than a CD8",
    "point on the same row records within-population centring. And six pairs is a small n,",
    "so an open marker leaves the question open. The six donors are the same count in",
    "every population and a different six people: Treg pairs JIA_patient_3 and skips 5,",
    "Tcon and CD8 do the reverse. Correlative; a footprint is inferred from target-gene",
    "expression."),
