...
save_overview(
  ...
  finding = sprintf(paste0("Scored with no favourite against all %s sets tested in the JIA Treg ",
                           ...
                           "reaches pooled FDR %s in Tcon on %d genes, so the cGAS-STING ",
  ...
  how_to_read = sprintf(paste(
    ...
    "pooled FDR alone; by NES the same sets order differently, and each ordering answers",
    "its own question. A cell reading 'not tested' had fewer than the minimum",
