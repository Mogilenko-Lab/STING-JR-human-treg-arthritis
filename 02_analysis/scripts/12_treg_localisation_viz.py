...
def main() -> None:
    ...
    save_overview(
        ...
        finding=(
            ...
            "WT_heat_up_purged_hypoxia run higher in synovial fluid than in peripheral blood. The "
            "cGAS-dependent Interaction_fdrOnly_up gate (18 genes) shows modest synovial-fluid "
            "elevation. The 7-gene Interaction_up gate is underpowered, and detection noise "
            "dominates it at per-cell resolution."
        ),
        script=SCRIPT,
        fn="build_figure",
        ...
        how_to_read=(
            "Box plots give per-cell AUCell score distributions for sorted CD4_Treg cells in "
            "peripheral blood (PB, blue) against synovial fluid (SF, vermillion). Solid lines "
            "are medians, dashed red lines means. Each panel declares its set sizes, nominal and "
            "in-dataset effective, and its power band. This panel is hypothesis-generating tier; "
            "donor-level pseudobulk DE carries the statistical claims."
        ),
        config=FIG_CFG,
        wide=True,
        height=6.2,
