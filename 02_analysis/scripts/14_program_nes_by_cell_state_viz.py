...
=============================================================================
The confirmatory companion to `arm_nes_by_cell_state`. Same geometry, same
cell-state colours, same filled-versus-open significance convention, same
annotation column, same tier line, so the two panels can be laid side by side
and read identically. Where that panel carries the three mouse-derived up arms,
this one carries the two program families a reader will ask about next, in two
labelled groups:

  Oxygen and HIF response      HALLMARK_HYPOXIA, GOBP_RESPONSE_TO_OXYGEN_LEVELS,
                               GOBP_CELLULAR_RESPONSE_TO_OXYGEN_LEVELS,
                               REACTOME_CELLULAR_RESPONSE_TO_HYPOXIA
  cGAS-STING and interferon    sting_specific_up, GOBP_CGAS_STING_SIGNALING_PATHWAY,
                               REACTOME_STING_MEDIATED_INDUCTION_OF_HOST_IMMUNE_RESPONSES,
                               WP_STING_PATHWAY_IN_KAWASAKILIKE_DISEASE_AND_COVID19,
                               ifn_only_up, HALLMARK_INTERFERON_ALPHA_RESPONSE

WHY THE FOUR OXYGEN SETS TRAVEL TOGETHER. They are four differently-built sets
for one named biology and they do not agree with each other, here or in the
mouse anchor. Drawing HALLMARK_HYPOXIA on its own invites a reader to take one
set's score as the family's, so the family is drawn.

NO RECOMPUTATION. Six of the ten rows are read from the committed
`tables/_overview/named_sets_in_sweep.csv`, which already carries verified
`nes`, `padj_pooled`, `set_size` and rank columns. The remaining four are read
from `tables/gsea_all.csv`, the same sweep output that table was built from.
Each output row records which table it came from. Nothing is derived a second
way, so two committed tables cannot disagree.

SET SIZE IS PART OF THE COMPARISON. A large set clears pooled significance far
more often than a small one in this sweep, so the gene count reaching each
population's ranked list is printed beside every dot and the sweep's own
per-band baseline rates are quoted in the caption.

ABSENCE IS DRAWN. A set that fell below `gsea_min_size` in a population has no
row in the sweep. Those cells are labelled `not tested in <population>` rather
than left blank, so an untested cell cannot be misread as a null result.

Input  (03_results/14_unbiased_enrichment/tables/):
  _overview/named_sets_in_sweep.csv    verified rows for six of the ten sets
...
Output (03_results/14_unbiased_enrichment/):
  ...
  tables/_overview/program_nes_by_cell_state.csv    the thirty plotted rows
...
# The two groups, in draw order. Rows inside a group are ordered by descending Treg
...
GROUP_GAP = 1.05          # blank data units inserted between the two groups
...
def wrap_set_id(set_id: str) -> str:
    ...
    for part in parts:
        ...
        if len(candidate) > ROW_LABEL_WRAP and cur:
            # The separator opens the continuation line rather than closing the one
...
def plotted_rows() -> pd.DataFrame:
    """The thirty cells behind the panel, each read from a committed sweep table.

    A set that `named_sets_in_sweep.csv` already verified is taken from there. The
    four that table does not carry are taken from `gsea_all.csv`, which is the sweep
    output that table was itself built from. A (set, population) cell with no row in
    either is carried with `tested = False` so the absence can be drawn.
    """
...
def assert_one_fdr_family(rows: pd.DataFrame) -> int:
    """Every plotted FDR is `padj_pooled`. Check it instead of trusting it.

    Six of the ten sets are read from `named_sets_in_sweep.csv` and four from
    `gsea_all.csv`. Both tables carry a per-collection `padj` alongside the pooled one,
...
def build_figure(rows: pd.DataFrame, order: list, fdr: float, width: float, height: float):
    ...
    # y positions, top row first, with a blank band between the two groups.
    ...
    fig.text(0.5, 0.972,
             "Donor pseudobulk NES by cell state, oxygen and cGAS-STING programs",
...
def main() -> None:
    ...
    save_overview(
        ...
        finding=(
            ...
            "reading taken from one of the four would be a reading the other three do not "
            "reproduce. In the second group the two interferon sets carry the strongest rows in "
            f"the figure, ifn_only_up reaching NES {float(ifn_cd8['nes']):.4f} at "
        ...
        how_to_read=(
            ...
            "pre-ranked fgsea. Rows are gene sets in two labelled groups, ordered inside a group "
            ...
            "cell carries an absence rather than a null. "
            ...
            "built sets for one named biology, and their scores differ, so the group is read as "
            "a family and a single member's score is read as that member's. The cGAS-STING "
            "group carries a positive result in Tcon, so the group is read set by set. "
            ...
            "entangled in cross-sectional human data, so these rows describe what the niche "
            "contrast contains and the question of what drives which stays open. A score of this "
            "kind has no interval, so none is drawn, and the reading stays correlative."
        ),
